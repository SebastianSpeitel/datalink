...
use crate::{Data, Request, Visitor, meta, value::Value};
...
pub trait Format {
    ...
    fn fmt_other(
        ...
        if !Self::HIDE_META && true {
            // todo: meta::META_TYPES.accepts(value) {
...
impl<W: Write, F: Format + ?Sized> DataFormatter<W, F> {
    ...
    pub fn new(buf: W) -> Self {
        Self::new_with_state(buf, F::init_state())
    }
...
impl<W: Write, F: Format + ?Sized> Visitor for DataFormatter<W, F> {
    #[inline]
...
impl<W: Write, F: Format + ?Sized> Request for DataFormatter<W, F> {
    #[inline]
    ...
    fn provide_link_unchecked<L: crate::Link>(&mut self, link: L) {
        ...
        let target = DataFormatter::<_, F>::new_with_state(target, target_state);
        let key = DataFormatter::<_, F>::new_with_state(key, key_state);

        link.query(key, target);
    }
...
impl<W: Write, F: Format + ?Sized> crate::request::ErasableRequest for DataFormatter<W, F> {
    #[inline]
    fn erased_link_request(
        ...
        (
            Some(ErasedRequest::Owned(Box::new(key))),
            Some(ErasedRequest::Owned(Box::new(target))),
        )
    }
    #[inline]
...
mod tests {
    ...
    //     unkeyed.query(DataFormatter::<_>::new(&mut debug_unkeyed));
    //     keyed.query(DataFormatter::<_>::new(&mut debug_keyed));
